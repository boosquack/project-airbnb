...
import { createBooking, resetCreateStatus } from '@/state/bookings/bookingsSlice';

const BookingForm = ({ listing, onSuccess }) => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { createStatus } = useSelector((state) => state.bookings);
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <DateRangePicker
              ...
              minDate={listing.availability?.from ? new Date(listing.availability.from) : new Date()}
              maxDate={listing.availability?.to ? new Date(listing.availability.to) : undefined}
              className="w-full"
            />
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <Stepper
              value={guests}
              onChange={(val) => setGuests(Math.max(1, Math.min(val, listing.maxGuests)))}
              label="Guest"
            />
          ...
          <Button type="submit" disabled={isSubmitting || nights === 0} className="w-full">
